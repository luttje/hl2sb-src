...
FileCompletionEdit::FileCompletionEdit( Panel *parent )
    : TextEntry( parent, NULL )
{
...
void FileOpenDialog::PopulateFileList()
{
  ...
  if ( m_DialogType != FOD_SELECT_DIRECTORY )
  {
    while ( ( filterPtr != NULL ) && ( *filterPtr != 0 ) )
    {
      ...
      while ( pszFileName )
      {
        if ( !g_pFullFileSystem->FindIsDirectory( findHandle ) || !IsOSX() || ( IsOSX() && g_pFullFileSystem->FindIsDirectory( findHandle ) && Q_stristr( pszFileName, ".app" ) ) )
        {
          char pFullPath[MAX_PATH];
  ...
  while ( pszFileName )
  {
    if ( pszFileName[0] != '.' && g_pFullFileSystem->FindIsDirectory( findHandle ) && ( !IsOSX() || ( IsOSX() && !Q_stristr( pszFileName, ".app" ) ) ) )
    {
      char pFullPath[MAX_PATH];
      Q_snprintf( pFullPath, MAX_PATH, "%s%s", currentDir, pszFileName );
