...
class HTMLComboBoxHost : public vgui::EditablePanel
{
 ...
 public:
  HTMLComboBoxHost( HTML *parent, const char *panelName )
...
void HTML::OnMouseReleased( MouseCode code )
{
  if ( code == MOUSE_LEFT )
  {
    ...
    if ( !m_sDragURL.IsEmpty() && input()->GetMouseOver() != GetVPanel() && input()->GetMouseOver() != NULL )
    {
      ...
      if ( ipanel()->RequestInfo( input()->GetMouseOver(), kv ) && kv->GetPtr( "AcceptPanel" ) != NULL )
      {
...
void HTML::OnKeyCodeTyped( KeyCode code )
{
  switch ( code )
  {
    ...
    case KEY_F:
    {
      if ( ( input()->IsKeyDown( KEY_LCONTROL ) || input()->IsKeyDown( KEY_RCONTROL ) ) || ( IsOSX() && ( input()->IsKeyDown( KEY_LWIN ) || input()->IsKeyDown( KEY_RWIN ) ) ) )
      {
...
//-----------------------------------------------------------------------------
HTML::CHTMLFindBar::CHTMLFindBar( HTML *parent )
    : EditablePanel( parent, "FindBar" )
{
  m_pParent = parent;
  m_bHidden = false;
