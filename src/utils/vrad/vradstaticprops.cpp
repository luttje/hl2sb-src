...
struct Rasterizer
{
  ...
  Rasterizer( Vector2D t0, Vector2D t1, Vector2D t2, size_t resX, size_t resY )
      : mT0( t0 ), mT1( t1 ), mT2( t2 ), mResX( resX ), mResY( resY ), mUvStepX( 1.0f / resX ), mUvStepY( 1.0f / resY )
  {
    Build();
  }

  CUtlVector< Location >::iterator begin()
  {
    return mRasterizedLocations.begin();
  }
  CUtlVector< Location >::iterator end()
  {
    return mRasterizedLocations.end();
  }

  void Build();

  inline size_t GetRow( float y ) const
  {
    return size_t( y * mResY );
  }
  inline size_t GetCol( float x ) const
  {
    return size_t( x * mResX );
  }
...
void Rasterizer::Build()
{
  ...
  int count = ( iMaxY - iMinY + 1 ) * ( iMaxX - iMinX + 1 );
...
void CVradStaticPropMgr::SerializeLighting()
{
  ...
  for ( int i = 0; i < count; ++i )
  {
    ...
    // allocate a buffer with enough padding for alignment
    size = sizeof( HardwareTexels::FileHeader_t ) + m_StaticProps[i].m_MeshData.Count() * sizeof( HardwareTexels::MeshHeader_t ) + totalTexelSizeBytes + 2 * kAlignment;
...
{
  Vector2D realPos = _barycentricCoord.x * _v[0] + _barycentricCoord.y * _v[1] + _barycentricCoord.z * _v[2];
      ...
      if ( pStrip->flags & OptimizedModel::STRIP_IS_TRILIST )
      {
        for ( int i = 0; i < pStrip->numIndices; i += 3 )
        {
          ...
          for ( auto it = rasterizer.begin(); it != rasterizer.end(); ++it )
          {
            ...
            if ( doWrite )
            {
              Vector itWorldPos = worldPos[0] * it->barycentric.x + worldPos[1] * it->barycentric.y + worldPos[2] * it->barycentric.z;

              Vector itWorldNormal = worldNormal[0] * it->barycentric.x + worldNormal[1] * it->barycentric.y + worldNormal[2] * it->barycentric.z;
