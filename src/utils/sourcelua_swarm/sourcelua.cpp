...
bool CSourceLua::Load( CreateInterfaceFn interfaceFactory, CreateInterfaceFn gameServerFactory )
{
  ...
  // This class isn't used in Alien Swarm
  ...
  lua_dofolder( LUA_PATH_ENUM );
  lua_dofolder( LUA_PATH_MODULES );
  lua_dofolder( LUA_PATH_INCLUDES );
  lua_dofolder( LUA_PATH_AUTORUN );
  lua_dofolder( LUA_PATH_AUTORUN_SERVER );
...
// Purpose: called when a client spawns into a server (i.e as they begin to play)
//---------------------------------------------------------------------------------
void CSourceLua::ClientFullyConnect( edict_t *pEntity )
{
  BEGIN_LUA_CALL_HOOK( "ClientActive" );
  CBasePlayer *pPlayer = ( CBasePlayer * )CBaseEntity_Instance( pEntity );
  lua_pushplayer( L, pPlayer );
