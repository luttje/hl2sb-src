...
class CInternalFileSystemPassThru : public Base
{
 public:
  ...
  {
    return m_pBaseFileSystemPassThru->SetFileWritable( pFileName, writable, pPathID );
  }
  ...
  {
    return m_pBaseFileSystemPassThru->ReadFile( pFileName, pPath, buf, nMaxBytes, nStartingByte, pfnAlloc );
  }
...
class CFileSystemPassThru : public CInternalFileSystemPassThru< IFileSystem >
{
 public:
  ...
  {
    m_pFileSystemPassThru->Disconnect();
  }
  ...
  {
    return m_pFileSystemPassThru->Init();
  }
  virtual void Shutdown()
  {
    m_pFileSystemPassThru->Shutdown();
  }
  ...
  {
    m_pFileSystemPassThru->FileTimeToString( pStrip, maxCharsIncludingTerminator, fileTime );
  }
  ...
  {
    return m_pFileSystemPassThru->LoadModule( pFileName, pPathID, bValidatedDllOnly );
  }
  ...
  {
    return m_pFileSystemPassThru->GetLocalPath( pFileName, pLocalPath, localPathBufferSize );
  }
  virtual bool FullPathToRelativePath( const char *pFullpath, char *pRelative, int maxlen )
  {
    return m_pFileSystemPassThru->FullPathToRelativePath( pFullpath, pRelative, maxlen );
  }
  ...
  {
    return m_pFileSystemPassThru->AsyncReadMultiple( pRequests, nRequests, pControls );
  }
  virtual FSAsyncStatus_t AsyncReadMultipleCreditAlloc( const FileAsyncRequest_t *pRequests, int nRequests, const char *pszFile, int line, FSAsyncControl_t *pControls )
  {
    return m_pFileSystemPassThru->AsyncReadMultipleCreditAlloc( pRequests, nRequests, pszFile, line, pControls );
  }
  ...
  {
    return m_pFileSystemPassThru->IsSteam();
  }
  ...
  virtual FSAsyncStatus_t AsyncAppend( const char *pFileName, const void *pSrc, int nSrcBytes, bool bFreeMemory, FSAsyncControl_t *pControl )
  {
    return m_pFileSystemPassThru->AsyncAppend( pFileName, pSrc, nSrcBytes, bFreeMemory, pControl );
  }
  ...
  {
    return m_pFileSystemPassThru->AsyncWrite( pFileName, pSrc, nSrcBytes, bFreeMemory, bAppend, pControl );
  }
  ...
  {
    return m_pFileSystemPassThru->AsyncWriteFile( pFileName, pSrc, nSrcBytes, bFreeMemory, bAppend, pControl );
  }
  virtual FSAsyncStatus_t AsyncAppendFile( const char *pDestFileName, const char *pSrcFileName, FSAsyncControl_t *pControl )
  {
    return m_pFileSystemPassThru->AsyncAppendFile( pDestFileName, pSrcFileName, pControl );
  }
  ...
  {
    return m_pFileSystemPassThru->AsyncResume();
  }
  // Next two functions each take an IAsyncFileFetch ptr.
  virtual void AsyncAddFetcher( void *pFetch )
  {
    return m_pFileSystemPassThru->AsyncAddFetcher( pFetch );
  }
  virtual void AsyncRemoveFetcher( void *pFetch )
  {
    return m_pFileSystemPassThru->AsyncRemoveFetcher( pFetch );
  }
  virtual const char *RelativePathToFullPath( const char *pFileName, const char *pPathID, char *pLocalPath, int localPathBufferSize, PathTypeFilter_t pathFilter = FILTER_NONE, PathTypeQuery_t *pPathType = NULL )
  {
    return m_pFileSystemPassThru->RelativePathToFullPath( pFileName, pPathID, pLocalPath, localPathBufferSize, pathFilter, pPathType );
  }
  virtual int GetSearchPath( const char *pathID, bool bGetPackFiles, char *pPath, int nMaxLen )
  {
    return m_pFileSystemPassThru->GetSearchPath( pathID, bGetPackFiles, pPath, nMaxLen );
  }
  ...
  {
    return m_pFileSystemPassThru->OpenEx( pFileName, pOptions, flags, pathID, ppszResolvedFilename );
  }
  ...
  {
    return m_pFileSystemPassThru->ReadFileEx( pFileName, pPath, ppBuf, bNullTerminate, bOptimalAlloc, nMaxBytes, nStartingByte, pfnAlloc );
  }
  ...
  {
    return m_pFileSystemPassThru->ExtractRootKeyName( type, outbuf, bufsize, filename, pPathID );
  }

  virtual bool GetFileTypeForFullPath( char const *pFullPath, wchar_t *buf, size_t bufSizeInBytes )
  {
    return m_pFileSystemPassThru->GetFileTypeForFullPath( pFullPath, buf, bufSizeInBytes );
  }

  virtual bool GetOptimalIOConstraints( FileHandle_t hFile, unsigned *pOffsetAlign, unsigned *pSizeAlign, unsigned *pBufferAlign )
  {
    return m_pFileSystemPassThru->GetOptimalIOConstraints( hFile, pOffsetAlign, pSizeAlign, pBufferAlign );
  }
  virtual void *AllocOptimalReadBuffer( FileHandle_t hFile, unsigned nSize, unsigned nOffset )
  {
  ...
  {
    m_pFileSystemPassThru->BeginMapAccess();
  }
  virtual void EndMapAccess()
  {
    m_pFileSystemPassThru->EndMapAccess();
  }
  ...
  virtual bool FullPathToRelativePathEx( const char *pFullPath, const char *pPathId, char *pRelative, int nMaxLen )
  {
    return m_pFileSystemPassThru->FullPathToRelativePathEx( pFullPath, pPathId, pRelative, nMaxLen );
  }
