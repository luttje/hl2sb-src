...
IMPLEMENT_NETWORKCLASS_ALIASED( BaseHL2MPBludgeonWeapon,
...
static const Vector g_bludgeonMins( -BLUDGEON_HULL_DIM, -BLUDGEON_HULL_DIM,
                                    -BLUDGEON_HULL_DIM );
...
void CBaseHL2MPBludgeonWeapon::Hit( trace_t &traceHit, Activity nHitActivity )
{
  ...
  if ( pHitEntity != NULL )
  {
...
#ifndef CLIENT_DLL
    CTakeDamageInfo info( GetOwner(), GetOwner(),
                          GetDamageForActivity( nHitActivity ), DMG_CLUB );
    ...
    TraceAttackToTriggers( info, traceHit.startpos, traceHit.endpos,
                           hitDirection );
#endif
...
Activity CBaseHL2MPBludgeonWeapon::ChooseIntersectionPointAndActivity(
    trace_t &hitTrace, const Vector &mins, const Vector &maxs,
    CBasePlayer *pOwner )
{
  int i, j, k;
  float distance;
  ...
  UTIL_TraceLine( vecSrc, vecHullEnd, MASK_SHOT_HULL, pOwner,
                  COLLISION_GROUP_NONE, &tmpTrace );
    ...
    for ( i = 0; i < 2; i++ )
    {
      for ( j = 0; j < 2; j++ )
      {
        for ( k = 0; k < 2; k++ )
        {
          ...
          UTIL_TraceLine( vecSrc, vecEnd, MASK_SHOT_HULL, pOwner,
                          COLLISION_GROUP_NONE, &tmpTrace );
          if ( tmpTrace.fraction < 1.0 )
          {
            float thisDistance =
...
// Input  : &traceHit -
//-----------------------------------------------------------------------------
bool CBaseHL2MPBludgeonWeapon::ImpactWater( const Vector &start,
                                            const Vector &end )
{
...
void CBaseHL2MPBludgeonWeapon::Swing( int bIsSecondary )
{
  ...
  UTIL_TraceLine( swingStart, swingEnd, MASK_SHOT_HULL, pOwner,
                  COLLISION_GROUP_NONE, &traceHit );
  Activity nHitActivity = ACT_VM_HITCENTER;

#ifndef CLIENT_DLL
  // Like bullets, bludgeon traces have to trace against triggers.
  CTakeDamageInfo triggerInfo( GetOwner(), GetOwner(),
                               GetDamageForActivity( nHitActivity ), DMG_CLUB );
  TraceAttackToTriggers( triggerInfo, traceHit.startpos, traceHit.endpos,
                         vec3_origin );
#endif
  ...
  if ( traceHit.fraction == 1.0 )
  {
    ...
    UTIL_TraceHull( swingStart, swingEnd, g_bludgeonMins, g_bludgeonMaxs,
