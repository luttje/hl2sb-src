...
#ifdef CLIENT_DLL
RecvPropBool( RECVINFO( m_bNeedPump ) ), RecvPropBool( RECVINFO( m_bDelayedFire1 ) ),
    RecvPropBool( RECVINFO( m_bDelayedFire2 ) ),
    RecvPropBool( RECVINFO( m_bDelayedReload ) ),
#else
SendPropBool( SENDINFO( m_bNeedPump ) ), SendPropBool( SENDINFO( m_bDelayedFire1 ) ),
    ...
    END_NETWORK_TABLE()
        ...
        BEGIN_PREDICTION_DATA( CWeaponShotgun )
            DEFINE_PRED_FIELD( m_bNeedPump, FIELD_BOOLEAN,
...
bool CWeaponShotgun::StartReload( void )
{
  ...
  // Tony; BUG BUG BUG!!! shotgun does one shell at a time!!! -- player model
  // only has a single reload!!! so I'm just going to dispatch the singular
  // for now.
...
bool CWeaponShotgun::Reload( void )
{
  ...
  FillClip();
  // Play reload on different channel as otherwise steals channel away from
  // fire sound
  WeaponSound( RELOAD );
...
void CWeaponShotgun::PrimaryAttack( void )
{
  ...
  FireBulletsInfo_t info( 7, vecSrc, vecAiming, GetBulletSpread(),
  ...
  QAngle punch;
  punch.Init( SharedRandomFloat( "shotgunpax", -2, -1 ),
              SharedRandomFloat( "shotgunpay", -2, 2 ), 0 );
...
void CWeaponShotgun::SecondaryAttack( void )
{
  ...
  FireBulletsInfo_t info( 12, vecSrc, vecAiming, GetBulletSpread(),
                          MAX_TRACE_LENGTH, m_iPrimaryAmmoType );
  info.m_pAttacker = pPlayer;
...
void CWeaponShotgun::ItemPostFrame( void )
{
  ...
  {
    // reload when reload is pressed, or if no buttons are down and weapon
    // is empty.
    StartReload();
  }
  else
...
void CWeaponShotgun::ItemHolsterFrame( void )
{
       ...
       sk_auto_reload_time.GetFloat() )
  {
    ...
    int ammoFill = MIN( ( GetMaxClip1() - m_iClip1 ),
                        GetOwner()->GetAmmoCount( GetPrimaryAmmoType() ) );
