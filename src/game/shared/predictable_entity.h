...
#define DECLARE_PREDICTABLE()                \
 public:                                     \
  ...
  template < typename T >                    \
  friend datamap_t *PredMapInit( T * )
#else
#define DECLARE_PREDICTABLE() template < typename T > \
friend datamap_t *PredMapInit( T * )
#endif
...
#define BEGIN_PREDICTION_DATA( className )                                      \
  ...
  datamap_t *className::GetPredDescMap( void )                                  \
  {                                                                             \
    return &m_PredMap;                                                          \
  }                                                                             \
...
#define BEGIN_PREDICTION_DATA_GUTS( className )                                                                                 \
  template < typename T >                                                                                                       \
  datamap_t *PredMapInit( T * );                                                                                                \
  template <>                                                                                                                   \
  datamap_t *PredMapInit< className >( className * );                                                                           \
...
#define BEGIN_PREDICTION_DATA( className )                  \
  template <>                                               \
  inline datamap_t *PredMapInit< className >( className * ) \
  {                                                         \
    if ( 0 )                                                \
    {                                                       \
      typedef className classNameTypedef;                   \
