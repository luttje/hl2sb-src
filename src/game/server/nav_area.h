...
class CNavArea : protected CNavAreaCriticalData
{
 public:
  ...
  unsigned int GetDebugID( void ) const
  {
    return m_debugid;
  }
  ...
  {
    m_place = place;
  }  // set place descriptor
  Place GetPlace( void ) const
  {
    return m_place;
  }  // get place descriptor
  ...
  }  // get areas connected TO this area by a ONE-WAY link (ie: we have no connection back to them)
  void AddIncomingConnection( CNavArea *source, NavDirType incomingEdgeDir );

  const NavLadderConnectVector *GetLadders( CNavLadder::LadderDirectionType dir ) const
  {
    return &m_ladder[dir];
  }
  CFuncElevator *GetElevator( void ) const
  {
    Assert( !( m_attributeFlags & NAV_MESH_HAS_ELEVATOR ) == ( m_elevator == NULL ) );
  ...
  int GetSpotEncounterCount( void ) const
  {
    return m_spotEncounters.Count();
  }
  ...
  {
    return m_seCorner.x - m_nwCorner.x;
  }
  float GetSizeY( void ) const
  {
    return m_seCorner.y - m_nwCorner.y;
  }
  ...
  }  // true if this area is a "battlefront" - where rushing teams initially meet
