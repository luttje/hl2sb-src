    ...
    class CBaseEntityModelLoadProxy
{
 protected:
  ...
  {
   public:
    explicit Handler( CBaseEntity *pEntity )
        : m_pEntity( pEntity ) {}
 ...
 public:
  explicit CBaseEntityModelLoadProxy( CBaseEntity *pEntity )
      : m_pHandler( new Handler( pEntity ) ) {}
  ~CBaseEntityModelLoadProxy()
  {
    delete m_pHandler;
  }
...
bool CBaseEntity::FVisible( CBaseEntity *pEntity, int traceMask, CBaseEntity **ppBlocker )
{
  ...
  if ( ( m_nWaterLevel != 3 && pEntity->m_nWaterLevel == 3 ) || ( m_nWaterLevel == 3 && pEntity->m_nWaterLevel == 0 ) )
    return false;
#endif
...
{
 public:
  CModelSoundsCacheSaver( const char *name )
