                      ...
                      public CAI_DefMovementSink
{
 ...
 public:
  ...
  {
    if ( !pEntToStepOff || !pEntToStepOff->IsWorld() ) m_hBigStepGroundEnt = pEntToStepOff;
  }
  ...
  const CAI_Network *GetNetwork() const
  {
    return m_pAINetwork;
  }
  void SetNetwork( CAI_Network *pNetwork )
  {
  ...
  {
    return !m_bNotOnNetwork;
  }

  void SetMaxRouteRebuildTime( float time )
  {
    m_timePathRebuildMax = time;
  }
