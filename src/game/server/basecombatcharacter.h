...
class CBaseCombatCharacter : public CBaseFlex
{
 ...
 public:
  ...
  {
    return ( interactionType > 0 ) ? HandleInteraction( interactionType, data, sourceEnt ) : false;
  }
  ...
  virtual bool AddPlayerItem( CBaseCombatWeapon *pItem )
  {
    return false;
  }
  ...
  void SetImpactEnergyScale( float fScale )
  {
    m_impactEnergyScale = fScale;
  }
...
#ifdef INVASION_DLL
// Powerup Inlines
inline bool CBaseCombatCharacter::CanBePoweredUp( void )
{
  return true;
}
...
inline void CBaseCombatCharacter::PowerupEnd( int iPowerup )
{
  return;
}
inline void CBaseCombatCharacter::PowerupThink( int iPowerup )
{
  return;
}
#endif
