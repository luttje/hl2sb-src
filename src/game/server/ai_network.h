...
{
  AI_NearNode_t() {}
  AI_NearNode_t( int index, float nodedist )
  {
    dist = nodedist;
    nodeIndex = index;
  }
...
class CNodeList : public CUtlPriorityQueue< AI_NearNode_t >
{
 public:
  ...
  CNodeList( int growSize = 0, int initSize = 0 )
      : CUtlPriorityQueue< AI_NearNode_t >( growSize, initSize, IsLowerPriority ) {}
  CNodeList( AI_NearNode_t *pMemory, int count )
      : CUtlPriorityQueue< AI_NearNode_t >( pMemory, count, IsLowerPriority ) {}
};
...
class CAI_Network : public IPartitionEnumerator
{
 public:
  ...
  int NearestNodeToPoint( CAI_BaseNPC *pNPC, const Vector &vecOrigin, bool bCheckVisiblity = true )
  {
    return NearestNodeToPoint( pNPC, vecOrigin, bCheckVisiblity, NULL );
  }
