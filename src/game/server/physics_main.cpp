...
// Purpose:
//-----------------------------------------------------------------------------
CPhysicsPushedEntities::CPhysicsPushedEntities( void )
    : m_rgPusher( 8, 8 ), m_rgMoved( 32, 32 )
{
  m_flMoveTime = -1.0f;
}
...
{
  VPROF_ENTER_SCOPE( ( !vprof_scope_entity_thinks.GetBool() ) ? "CBaseEntity::PhysicsDispatchThink" : EntityFactoryDictionary()->GetCannonicalName( GetClassname() ) );
...
{
  VPROF( ( !vprof_scope_entity_gamephys.GetBool() ) ? "Physics_SimulateEntity" : EntityFactoryDictionary()->GetCannonicalName( pEntity->GetClassname() ) );
  ...
  if ( pEntity->edict() )
  {
    ...
    if ( pEntity->m_PredictableID->IsActive() )
    {
      ...
      {
        VPROF( ( !vprof_scope_entity_gamephys.GetBool() ) ? "pEntity->PhysicsSimulate" : EntityFactoryDictionary()->GetCannonicalName( pEntity->GetClassname() ) );
