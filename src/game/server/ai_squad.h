...
class CAI_Squad
{
 public:
  ...
  float GetSquadSoundWaitTime() const
  {
    return m_flSquadSoundWaitTime;
  }
  void SetSquadSoundWaitTime( float time )
  {
 ...
 private:
  ...
  const AISquadEnemyInfo_t *FindEnemyInfo( CBaseEntity *pEnemy ) const
  {
    return const_cast< CAI_Squad * >( this )->FindEnemyInfo( pEnemy );
  }
