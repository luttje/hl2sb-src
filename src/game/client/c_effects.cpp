...
void CClient_Precipitation::CreateAshParticle( void )
{
  ...
  while ( m_tAshParticleTimer.NextEvent( curTime ) )
  {
    ...
    if ( offset[0] > absmaxs[0] || offset[1] > absmaxs[1] || offset[2] > absmaxs[2] || offset[0] < absmins[0] || offset[1] < absmins[1] || offset[2] < absmins[2] )
      continue;
...
// Output : Vector
//-----------------------------------------------------------------------------
CEmberEmitter::CEmberEmitter( const char *pDebugName )
    : CSimpleEmitter( pDebugName )
{
}
