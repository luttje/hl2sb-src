...
class CParticleEffectBinding : public CDefaultClientRenderable
{
 ...
 public:
  ...
  int GetRemoveFlag()
  {
    return GetFlag( FLAGS_REMOVE );
  }
  void SetRemoveFlag()
  {
    SetFlag( FLAGS_REMOVE, 1 );
  }
  ...
  int GetAlwaysSimulate()
  {
    return GetFlag( FLAGS_ALWAYSSIMULATE );
  }
  ...
  void SetIsNewParticleSystem( void )
  {
    SetFlag( FLAGS_NEW_PARTICLE_SYSTEM, 1 );
  }
  ...
  // overhead.
  int WasDrawnPrevFrame()
  {
    return GetFlag( FLAGS_DRAWN_PREVFRAME );
  }
  ...
  int IsEffectCameraSpace()
  {
    return GetFlag( FLAGS_CAMERASPACE );
  }
  ...
  int GetAutoUpdateBBox()
  {
    return GetFlag( FLAGS_AUTOUPDATEBBOX );
  }
 ...
 private:
  ...
  int GetRemovalInProgressFlag()
  {
    return GetFlag( FLAGS_REMOVALINPROGRESS );
  }
  void SetRemovalInProgressFlag()
  {
    SetFlag( FLAGS_REMOVALINPROGRESS, 1 );
  }

  // BBox is recalculated before it's put into the tree for the first time.
  int GetNeedsBBoxUpdate()
  {
    return GetFlag( FLAGS_NEEDS_BBOX_UPDATE );
  }
  ...
  int GetFirstFrameFlag()
  {
    return GetFlag( FLAGS_FIRST_FRAME );
  }
  ...
  int WasDrawn()
  {
    return GetFlag( FLAGS_DRAWN );
  }
  void SetDrawn( int bDrawn )
  {
    SetFlag( FLAGS_DRAWN, bDrawn );
  }
