                               ...
                               const FSEventStreamEventFlags eventMasks[], const FSEventStreamEventId eventIDs[] )
{
  ...
  for ( int i = 0; i < numEvents; i++ )
  {
    ...
    if ( eventMasks[i] & kFSEventStreamEventFlagMustScanSubDirs || eventMasks[i] & kFSEventStreamEventFlagUserDropped || eventMasks[i] & kFSEventStreamEventFlagKernelDropped )
    {
      bRecurse = true;
    }
