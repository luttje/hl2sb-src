...
struct TelemetryGPUStats_t
{
  ...
  {
    return m_nTotalBufferLocksAndUnlocks + m_nTotalTexLocksAndUnlocks + m_nTotalBlit2 + m_nTotalResolveTex + m_nTotalPresent;
  }
};
...
{
  GLenum func;
  inline bool operator==( const GLDepthFunc_t &src ) const
  {
    return EQ( func );
  }
};
...
{
  char mask;
  inline bool operator==( const GLDepthMask_t &src ) const
  {
    return EQ( mask );
  }
};
...
{
  GLdouble d;
  inline bool operator==( const GLClearDepth_t &src ) const
  {
    return EQ( d );
  }
};
...
{
  GLint s;
  inline bool operator==( const GLClearStencil_t &src ) const
  {
    return EQ( s );
  }
};
...
class CPinnedMemoryBuffer
{
 ...
 public:
  ...
  inline uint GetOfs() const
  {
    return m_nOfs;
  }
  inline uint GetBytesRemaining() const
  {
    return m_nSize - m_nOfs;
  }
...
class GLMContext
{
 public:
  ...
  FORCEINLINE void WriteAlphaToCoverageEnable( GLAlphaToCoverageEnable_t *src )
  {
  ...
  FORCEINLINE void WriteClipPlaneEquation( GLClipPlaneEquation_t *src, int which )
  {
  ...
  FORCEINLINE void WriteColorMaskMultiple( GLColorMaskMultiple_t *src, int which )
  {
 ...
 protected:
  ...
  FORCEINLINE void ReleasedShader()
  {
    NullProgram();
  }
  ...
  FORCEINLINE void NewLinkedProgram()
  {
    ClearCurAttribs();
  }
