...
#define RDPMC( counter, var ) \
  _asm mov ecx, counter _asm RDPMC _asm mov DWORD PTR var, eax _asm mov DWORD PTR var + 4, edx

// RDPMC Instruction macro, for performance counter 1 (ecx = 1)
#define RDPMC0( var ) \
  _asm mov ecx, 0 _asm RDPMC _asm mov DWORD PTR var, eax _asm mov DWORD PTR var + 4, edx

#define RDPMC1( var ) \
  _asm mov ecx, 1 _asm RDPMC _asm mov DWORD PTR var, eax _asm mov DWORD PTR var + 4, edx
