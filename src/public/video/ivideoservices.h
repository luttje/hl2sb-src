...
class VideoFrameRate_t
{
 public:
  ...
  inline VideoFrameRate_t( int FPS, bool NTSC )
  {
    SetFPS( FPS, NTSC );
  }
  inline explicit VideoFrameRate_t( float FPS )
  {
    SetFPS( FPS );
  };
  ...
  inline VideoFrameRate_t( const VideoFrameRate_t &rhs )
  {
    *this = rhs;
  };
  ...
  {
    return ( int )( ( float )m_TimeUnitsPerSecond / ( float )m_TimeUnitsPerFrame + 0.5f );
  }
  inline bool IsNTSCRate() const
  {
    return ( m_TimeUnitsPerFrame == 1001 );
  }
  inline int GetUnitsPerSecond() const
  {
    return m_TimeUnitsPerSecond;
  }
  inline int GetUnitsPerFrame() const
  {
    return m_TimeUnitsPerFrame;
  }
