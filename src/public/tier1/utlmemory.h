...
#define UTLMEMORY_TRACK_FREE() \
  if ( !m_pMemory )            \
    ...
    MemAlloc_RegisterDeallocation( "Sum of all UtlMemory", 0, m_nAllocationCount * sizeof( T ), m_nAllocationCount * sizeof( T ), 0 )
#else
...
class CUtlMemoryConservative
{
 public:
  ...
  CUtlMemoryConservative( T* pMemory, int numElements )
  {
    Assert( 0 );
  }
  ...
  Iterator_t Next( const Iterator_t& it ) const
  {
    return Iterator_t( ( it.index + 1 < it.limit ) ? it.index + 1 : InvalidIndex(), it.limit );
  }
  int GetIndex( const Iterator_t& it ) const
  {
    return it.index;
