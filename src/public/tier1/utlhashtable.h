...
class CUtlHashtable
{
 ...
 protected:
  ...
  template < typename KeyParamT >
  handle_t DoInsert( KeyParamT k, typename ArgumentTypeInfo< ValueT >::Arg_t v, unsigned int h, bool *pDidInsert );
  template < typename KeyParamT >
  handle_t DoInsertNoCheck( KeyParamT k, typename ArgumentTypeInfo< ValueT >::Arg_t v, unsigned int h );
 ...
 public:
  ...
  CUtlHashtable( entry_t *pMemory, unsigned int nCount, const KeyHashT &hash = KeyHashT(), KeyIsEqualT const &eq = KeyIsEqualT() )
      : m_nUsed( 0 ), m_nMinSize( 8 ), m_bSizeLocked( false ), m_eq( eq ), m_hash( hash )
  {
  ...
  KeyIsEqualT &GetEqualRef()
  {
    return m_eq;
  }
  KeyHashT const &GetHashRef() const
  {
    return m_hash;
  }
  ...
  {
    return ( unsigned )idx < ( unsigned )m_table.Count() && m_table[idx].IsValid();
  }
  ...
  // Iteration functions
  handle_t FirstHandle() const
  {
    return NextHandle( ( handle_t )-1 );
  }
  ...
  bool HasElement( KeyArg_t k ) const
  {
    return InvalidHandle() != Find( k );
  }
  bool HasElement( KeyAlt_t k ) const
  {
    return InvalidHandle() != Find( k );
  }
  ...
  handle_t Insert( KeyArg_t k, ValueArg_t v, unsigned int hash, bool *pDidInsert = NULL )
  {
    Assert( hash == m_hash( k ) );
  ...
  handle_t Insert( KeyAlt_t k, ValueArg_t v, unsigned int hash, bool *pDidInsert = NULL )
  {
    Assert( hash == m_hash( k ) );
  ...
  bool Remove( KeyArg_t k )
  {
    return DoRemove< KeyArg_t >( k, m_hash( k ) ) >= 0;
  }
  ...
  bool Remove( KeyAlt_t k )
  {
    return DoRemove< KeyAlt_t >( k, m_hash( k ) ) >= 0;
  }
  ...
  KeyT const &Key( handle_t idx ) const
  {
    return m_table[idx]->m_key;
  }
  Element_t const &Element( handle_t idx ) const
  {
    return m_table[idx]->GetValue();
  }
  Element_t &Element( handle_t idx )
  {
    return m_table[idx]->GetValue();
  }
  ...
  Element_t &operator[]( handle_t idx )
  {
    return m_table[idx]->GetValue();
  }
  ...
  {
    Assert( m_eq( m_table[idx]->m_key, k ) && m_hash( k ) == m_hash( m_table[idx]->m_key ) );
    m_table[idx]->m_key = k;
  }
...
class CUtlStableHashtable
{
 ...
 public:
  ...
  UtlHashHandle_t Insert( KeyArg_t k )
  {
    return DoInsert< KeyArg_t >( k );
  }
  UtlHashHandle_t Insert( KeyAlt_t k )
  {
    return DoInsert< KeyAlt_t >( k );
  }
  ...
  UtlHashHandle_t Find( KeyArg_t k ) const
  {
    return DoFind< KeyArg_t >( k );
  }
  UtlHashHandle_t Find( KeyAlt_t k ) const
  {
    return DoFind< KeyAlt_t >( k );
  }
  bool Remove( KeyArg_t k )
  {
    return DoRemove< KeyArg_t >( k );
  }
  bool Remove( KeyAlt_t k )
  {
    return DoRemove< KeyAlt_t >( k );
  }
  ...
  typedef typename KVPair::ValueReturn_t Element_t;
  KeyT const &Key( UtlHashHandle_t idx ) const
  {
    return m_data[idx].m_key;
  }
  Element_t const &Element( UtlHashHandle_t idx ) const
  {
    return m_data[idx].GetValue();
  }
  Element_t &Element( UtlHashHandle_t idx )
  {
    return m_data[idx].GetValue();
  }
  ...
  {
    Assert( GetEqualRef()( m_data[idx].m_key, k ) && GetHashRef()( k ) == GetHashRef()( m_data[idx].m_key ) );
    m_data[idx].m_key = k;
  }
  ...
  UtlHashHandle_t InvalidHandle() const
  {
 ...
 protected:
  ...
  struct IndirectIndex
  {
    explicit IndirectIndex( IndexStorage_t i )
        : m_index( i ) {}
  ...
  struct HashProxy
  {
    ...
    unsigned int operator()( KeyArg_t k ) const
    {
      return m_hash( k );
    }
    unsigned int operator()( KeyAlt_t k ) const
    {
      return m_hash( k );
    }
  };
